...
To calculate this key, you will need to generate a **SHA512** hash by concatenating your secret key and merchant ID. This hash is then compared with the "hash-key" found in the response header to verify the integrity of the request before processing it.
...
| `wallet.transaction.status`                |  Event triggered when a wallet transaction occurred  |
...
<Tabs>
  ...
  <Tab title="Wallet transaction status">
  ```json
    {
      "data": {
          "moneyMovement": "Credit",
          "transaction": {
              "originatorAccountNumber": "3039713406",
              "originatorBankName": "First bank",
              "originatorName": "Tee Lee",
              "currency": "NGN",
              "amount": "1000.00",
              "paymentReference": "vvvvvvbnocccbbvvvcnobvvvvv",
              "destinationAccountName": "One Director",
              "destinationAccountNumber": "8053660290"
          }
      },
      "eventType": "wallet.transaction.status",
      "sentAt": "2025-03-03T17:59:41.849167"
   }
  ```
  </Tab>
</Tabs>
